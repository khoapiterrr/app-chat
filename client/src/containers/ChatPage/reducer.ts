...
const messageReducer = (state = initialState, { type, payload }: Action) =>
  produce(state, (draft) => {
    let currentUser: User, message: any;
    switch (type) {
      ...
      case constants.SOCKET_SENT_MESSAGE:
        ...
        if (
          currentUser._id === message.receiver._id ||
        ...
        if (
          (state.record &&
            state.record.receiver._id === message.sender._id &&
            currentUser._id === message.receiver._id) ||
          (state.record &&
            state.record.receiver._id === message.receiver._id &&
            currentUser._id === message.sender._id)
        ) {
      ...
      case constants.CHAT_GET_IMAGE_LIST_SUCCESS:
        ...
        let tempImages = payload.images.map((image: any) => {
          return {
      ...
      case constants.CHAT_GROUP_LEAVE:
        draft.messages = draft.messages.filter(
          (item: any) => item.receiver._id !== payload,
        );
        draft.record = null;
        break;
      case constants.CHAT_GROUP_ADD_MEMBERS_SUCCESS:
        draft.record.receiver.members =
          draft.record.receiver.members.concat(payload);
        break;
      case constants.CHAT_GROUP_CHANGE_AVATAR:
        draft.record.receiver.picture = payload.picture;
        state.messages.forEach((item: any, index: any) => {
          if (item.receiver._id === payload.groupId) {
            draft.messages[index].receiver.picture = payload.picture;
          }
        });
        break;
      case constants.CHAT_GROUP_UPDATE_SUCCESS:
        ...
        break;
      case constants.SHOW_USER_INFO:
        draft.isShowUserInfo = true;
        break;
      case constants.hIDE_USER_INFO:
        draft.isShowUserInfo = false;
        break;
      case constants.TOGGLE_USER_INFO:
        draft.isShowUserInfo = !state.isShowUserInfo;
        break;
      default:
        break;
    }
